class CatalogCleaner < Object
  ...
  def self.set_false_to_nil_booleans
    sql = 'UPDATE products SET is_mix_box = FALSE WHERE is_mix_box IS NULL'
    ActiveRecord::Base.connection.execute(sql)

    sql = 'UPDATE products SET units_sale_allowed = FALSE WHERE units_sale_allowed IS NULL'
    ActiveRecord::Base.connection.execute(sql)

    sql = 'UPDATE orders SET iva = FALSE WHERE iva IS NULL'
    ActiveRecord::Base.connection.execute(sql)
  end



  # PARA ABJAO SON EJEMPLOS DE LO QUE SE PUEDE HACER
